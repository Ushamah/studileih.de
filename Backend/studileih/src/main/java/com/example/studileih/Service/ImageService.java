...
public class ImageService {
    ...
    private final Path rootLocation = Paths.get("C:\\Users\\Boommeister69\\workspace\\studileih.de\\Backend\\studileih\\src\\image");

    public void store(MultipartFile file) {
        try {
            Files.copy(file.getInputStream(), this.rootLocation.resolve(file.getOriginalFilename()));
        } catch (Exception e) {
            throw new RuntimeException("storageService.store(): FAIL! Please don't upload the same picture twice -> photo already exists.");
        }
    }
